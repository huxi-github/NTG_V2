...
static const u_char *ntg_md5_body(ntg_md5_t *ctx, const u_char *data,
    size_t size);


void
ntg_md5_init(ntg_md5_t *ctx)
{
    ctx->a = 0x67452301;
    ctx->b = 0xefcdab89;
    ctx->c = 0x98badcfe;
...
void
ntg_md5_update(ntg_md5_t *ctx, const void *data, size_t size)
{
    size_t  used, free;
    ...
    if (used) {
        ...
        if (size < free) {
            ntg_memcpy(&ctx->buffer[used], data, size);
            return;
        }

        ntg_memcpy(&ctx->buffer[used], data, free);
        data = (u_char *) data + free;
        size -= free;
        (void) ntg_md5_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = ntg_md5_body(ctx, data, size & ~(size_t) 0x3f);
        size &= 0x3f;
    }

    ntg_memcpy(ctx->buffer, data, size);
}


void
ntg_md5_final(u_char result[16], ntg_md5_t *ctx)
{
    size_t  used, free;
    ...
    if (free < 8) {
        ntg_memzero(&ctx->buffer[used], free);
        (void) ntg_md5_body(ctx, ctx->buffer, 64);
        used = 0;
        free = 64;
    }

    ntg_memzero(&ctx->buffer[used], free - 8);
...
static const u_char *
ntg_md5_body(ntg_md5_t *ctx, const u_char *data, size_t size)
{
    uint32_t       a, b, c, d;
